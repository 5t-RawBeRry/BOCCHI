...
export async function processPotChestObservationById(
  env: DatabaseEnv,
...
export async function processPendingPotChestObservations(env: DatabaseEnv): Promise<ProcessorResult> {
...
async function processOne(db: D1Database, observation: PendingObservation): Promise<ProcessorResult> {
  try {
    ...
    console.error(`Failed to process pot-chest observation ${observation.id}`, error);
...
async function findCandidate(
  ...
  `).bind(
    ...
    observation.pot_fate_id,
    observation.is_reroll,
    observation.world_x,
    CLUSTER_RADIUS,
    observation.world_y,
    CLUSTER_RADIUS,
...
async function createCandidate(
  ...
  const result = await db.prepare(`
    INSERT INTO pot_chest_candidates (
      territory_id,
      pot_fate_id,
      is_reroll,
      centroid_x,
  ...
  if (!result.success || result.meta.last_row_id === undefined) {
    throw new Error("Pot-chest candidate insert failed.");
  }
...
async function assignObservation(
  ...
  const inserted = await db.prepare(`
    INSERT OR IGNORE INTO pot_chest_candidate_members (
      candidate_id,
      observation_id,
      installation_hash
    ) VALUES (?, ?, ?)
  ...
  if (!inserted.success) {
    throw new Error("Pot-chest member insert failed.");
  }
    ...
    db.prepare(`
      WITH delta AS (
        SELECT IIF(
          EXISTS (
            SELECT 1
            FROM pot_chest_candidate_members
            WHERE candidate_id = ?
  ...
  if (previous === undefined) {
    throw new Error("Pot-chest candidate missing during assign.");
  }
